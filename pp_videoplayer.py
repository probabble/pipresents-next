...
# ***************************************
    ...
    def __init__(self, show_id, root, canvas, show_params, track_params, pp_dir, pp_home, pp_profile):
        ...
        if self.omx_audio:
            self.omx_audio = "-o " + self.omx_audio
        ...
        self.pim = PluginManager(self.show_id, self.root, self.canvas, self.show_params, self.track_params, self.pp_dir, 
    ...
    def play(self, track, showlist, end_callback, ready_callback, enable_menu=False):
        ...
        self.show_manager = ShowManager(self.show_id, self.showlist, self.show_params, self.root, self.canvas, 
                                        self.pp_dir, self.pp_profile, self.pp_home)
        ...
        self.omx_window = '--win "{x1} {y1} {x2} {y2}" '.format(x1=x1, y1=y1, x2=x2, y2=y2) if has_window else ''
    ...
    def input_pressed(self, symbol):
        """
        evaluates the meaning of input and handles it
        """
        if symbol.startswith('omx-'):
...
# ***************************************
    ...
    def stop(self):
        """sends signal to stop the track to the state machine"""
...
# **********************
    ...
    def start_play_state_machine(self, track):
        ...
        options = "{audio} {volume} {window} {omx_options} ".format(audio=self.omx_audio, 
                                                                    volume=self.omx_volume, 
                                                                    window=self.omx_window, 
                                                                    omx_options=self.show_params['omx-other-options'])
    ...
    def display_content(self):
        ...
        if self.background_file:
            ...
            if not os.path.exists(self.background_img_file):
                ...
                self.drawn = self.canvas.create_image(w, h, image=self.background, anchor=CENTER, tag='pp-content')
        ...
        if self.show_params['show-text'] != '' and self.track_params['display-show-text'] == 'yes':
            x = int(self.show_params['show-text-x'])
            y = int(self.show_params['show-text-y'])
        ...
        if self.track_params['track-text'] != '':
            self.canvas.create_text(int(self.track_params['track-text-x']),
...
# warp _ or xy2
    ...
    def parse_window(self, line):
            ...
            if len(fields) == 5:
                ...
                if not all(field.isdigit() for field in fields[:1]):
...
# *****************
# TODO: update this, and separate it into a tests directory
